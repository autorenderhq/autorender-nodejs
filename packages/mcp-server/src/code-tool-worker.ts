...
import { Autorender, ClientOptions } from '@autorender/nodejs';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Autorender)`
    : `const run: (${functionSource.client}: Autorender) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
