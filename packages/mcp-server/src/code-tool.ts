...
import { ClientOptions } from '@autorender/nodejs';

const prompt = `Runs JavaScript code to interact with the Autorender API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    AUTORENDER_API_KEY: readEnv('AUTORENDER_API_KEY') ?? client.apiKey ?? undefined,
    AUTORENDER_BASE_URL: readEnv('AUTORENDER_BASE_URL') ?? client.baseURL ?? undefined,
  };
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@autorender/nodejs';
