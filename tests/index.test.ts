...
describe('logging', () => {
  ...
  test('default logLevel is warn', async () => {
    const client = new Autorender({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    const client = new Autorender({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Autorender({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'AUTORENDER_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Autorender({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    test('explicit option', () => {
      const client = new Autorender({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Autorender({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Autorender({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Autorender({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['AUTORENDER_API_KEY'] = 'My API Key';
    const client = new Autorender();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['AUTORENDER_API_KEY'] = 'another My API Key';
    const client = new Autorender({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Autorender({ apiKey: 'My API Key', fetch: testFetch });
