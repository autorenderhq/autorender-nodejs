...
import { AbstractPage, type PagePaginationParams, PagePaginationResponse } from './core/pagination';
...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['AUTORENDER_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['AUTORENDER_LOG'] or 'warn' if it isn't set.
   */
...
export class Autorender {
  ...
  constructor({
    ...
    const options: ClientOptions = {
      apiKey,
      ...opts,
      baseURL: baseURL || `https://upload.autorender.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? Autorender.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('AUTORENDER_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://upload.autorender.io';
  }
  ...
  >(
    ...
    return new Pagination.PagePromise<PageClass, Item>(this as any as Autorender, request, Page);
  }
